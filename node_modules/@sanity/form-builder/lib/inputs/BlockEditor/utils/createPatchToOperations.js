...
function createPatchesToChange(blockContentFeatures, blockContentType) {
  ...
  function patchSpanText(patch, editor, node, blockNode) {
    ...
    if (patch.type === 'set') {
      ...
      var patchSpan = valueIsString ? null : patch.value;
      var patchText = valueIsString ? patch.value : patchSpan.text; // If single leaf, we can just replace the text with the current marks
      ...
      if (textNode.leaves.size === 1) {
        ...
        if (valueIsString) {
          ...
          marks = _slate.Mark.createSet(patchSpan.marks.map(mark => ({
            type: mark
          })));
        }
      ...
      workTextNode.leaves[leafIndex] = {
        object: 'leaf',
        text: patchText,
        marks: valueIsString ? node.leaves.get(leafIndex).marks : _slate.Mark.createSet(patchSpan.marks.map(mark => ({
          type: mark
        })))
      }; // Replace it
