...
function getDocumentTypeList(typeNameOrSpec, sanitySchema) {
  ...
  return new _DocumentTypeList.DocumentTypeListBuilder().id(spec.id || typeName).title(spec.title || title).filter('_type == $type').params({
    ...
    id: 'actions',
    title: 'Actions'
  }]).child(spec.child || (documentId => new _Document.DocumentBuilder().id('editor').schemaType(type).documentId(documentId))).canHandleIntent(spec.canHandleIntent || _Intent.defaultIntentChecker).menuItems(spec.menuItems || [// Create new (from action button) will be added in serialization step of GenericList
  // Sort by <Y>
