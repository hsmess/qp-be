...
function parseChunks(chunks) {
  ...
  return chunks.reduce((pane, chunk) => {
    if (isParam(chunk)) {
      ...
      pane.params = _objectSpread({}, pane.params, {
        [key]: value
      });
...
function encodeChunks(pane, i, group) {
  var payload = pane.payload,
      ...
      id = pane.id;
  var sameAsFirst = i !== 0 && id === group[0].id;
  var encodedPayload = typeof payload === 'undefined' ? undefined : btoa(JSON.stringify(payload));
  var encodedParams = Object.keys(params).reduce((pairs, key) => {
    if (sameAsFirst && i !== 0 && !_PaneRouterContext.exclusiveParams.includes(key) && group[0].params[key] === params[key]) {
      return pairs;
    }

    return params[key] ? [...pairs, "".concat(key, "=").concat(params[key])] : pairs;
  }, []);
  return [sameAsFirst ? '' : id].concat([encodedParams.length > 0 && encodedParams, encodedPayload].filter(Boolean)).join(',') || ',';
}
...
function parsePanesSegment(str) {
  ...
  return str.split(';').map(group => group.split('|').map(segment => {
  ...
  }).map((pane, i, siblings) => pane.id ? pane : _objectSpread({}, pane, {
    id: siblings[0].id
