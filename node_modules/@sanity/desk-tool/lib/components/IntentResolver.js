...
var _StructureError = _interopRequireDefault(require("./StructureError"));
...
var IntentResolver = _react.default.memo(function IntentResolver(_ref) {
  ...
  var _ref2 = params || {},
      ...
      id = _ref2.id,
      otherParams = _objectWithoutProperties(_ref2, ["type", "id"]);
  ...
  if (!documentType) {
    return isLoaded ? _react.default.createElement(Redirect, {
      panes: [[{
        id: "__edit__".concat(id || (0, _uuid.default)()),
        params: {}
      }]]
  ...
  var isLoading = !structure || structure.some(item => item === _.LOADING_PANE);
...
function getNewRouterState(_ref3) {
  ...
  var fallbackParameters = {
    type,
    template: params.template
  };
  var newDocumentId = documentId === FALLBACK_ID ? (0, _uuid.default)() : documentId;
  return terminatesInDocument ? paneSegments.slice(0, -1).concat([lastGroup.slice(0, -1).concat(_objectSpread({}, lastSibling, {
    id: newDocumentId
  }))]) : [[{
...
function useDocumentType(documentId, specifiedType) {
  ...
  (0, _react.useEffect)(() => {
    var sub = resolveTypeForDocument(documentId, specifiedType).subscribe(documentType => setDocumentType({
      documentType,
      isLoaded: true
    }));
